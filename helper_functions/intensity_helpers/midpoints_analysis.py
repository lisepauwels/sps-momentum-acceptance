...
def plot_midpoints(midpoints, midpoints_md=None, line_types=None, planes=None, chromas=None, colours=None, line_styles=None, markers=None, is_reversed=True, ylim=None, savefig=None, fontsize=16):
    ...
    ax.grid()
    ax.legend(ncols=2 if len(planes) > 1 else 1, fontsize=fontsize - 2, frameon=True)
    ax.set_title(f"Midpoints vs Chromaticity -- {planes[0] if len(planes) == 1 else 'Both Planes'}", fontsize=fontsize)
    plt.tight_layout()
