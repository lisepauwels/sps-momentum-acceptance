...
def plot_intensity_drop(normalised_intensity, line_types=None, chromas=None, line_styles=None, cmap=None, savefig=None, fontsize=16, legend=True):
    ...
    if legend:
        ...
        legend_model = ax.legend(handles=line_handles, title="Model", loc="upper left", frameon=True, fontsize=fontsize - 2)
        ax.add_artist(legend_model)

    sm = mpl.cm.ScalarMappable(norm=norm, cmap=cmap)
    sm.set_array([])  # required by older mpl for colorbar
    cbar = fig.colorbar(sm, ax=ax, pad=0.02)
    cbar.set_label(r"Normalised Chromaticity  $\xi$", fontsize=fontsize)
