...
def _is_skew_resonance(m: int, n: int) -> bool:
    """
    Classify a resonance line using the RDT-parity rule:

        (p-q) Qx + (r-t) Qy = integer

    with resonance order |p-q| + |r-t|.

    For order >= 2, resonances with an odd vertical coefficient are treated
    as skew; resonances with an even vertical coefficient are treated as
    normal. First-order resonances are always treated as non-skew.
    """
    order = abs(m) + abs(n)
    if order <= 1:
        return False
...
class TuneDiagram:
    ...
    def plot(
        ...
        for order in range(1, self.max_order + 1):
            ...
            mn_normal = [(m, n) for m, n in mn_all if not _is_skew_resonance(m, n)]
            mn_skew   = [(m, n) for m, n in mn_all if _is_skew_resonance(m, n)]
    ...
    def finalize(
        ...
        ax.set_xlabel(xlabel or r"$Q_x$", fontsize=fontsize)
        ax.set_ylabel(ylabel or r"$Q_y$", fontsize=fontsize)
