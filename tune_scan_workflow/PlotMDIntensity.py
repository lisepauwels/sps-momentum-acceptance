...
# ── path setup ────────────────────────────────────────────────────────────────
HELPER_DIR = Path('/Users/lisepauwels/sps-offmom-scans')
TUNE_DIR   = Path('/Users/lisepauwels/sps_simulations/Studies/MomentumAcceptance/HelperFunctions')
...
DATA_ROOT = HELPER_DIR / 'results'
MAP_ROOT  = Path('SweepTrajectoryMaps')

# Active scans
SCANS = [QX_SCAN, QY_SCAN, CHROMA_SCAN_Y, CHROMA_SCAN_X]
...
OUTPUT_ROOT = Path('plots_md_intensity')
...
def load_tune_map(scan_cfg: dict, key, map_case: str) -> TuneMap | None:
    ...
    xi_x, xi_y = scan_key_to_chroma(scan_cfg, key)
    fname = _map_filename(qx, qy, xi_x=xi_x, xi_y=xi_y)
    path = MAP_ROOT / scan_label / map_case / fname
    if not path.exists():
...
def _td_and_fig(scan, scan_vals, n_cbars=1):
    """Create TuneDiagram + figure with n_cbars inset colorbar axes."""
    ...
    caxes = [ax.inset_axes(pos) for pos in colorbar_inset_positions(n_cbars)]
...
def plot2_coloured_by_intensity(all_data, scan_vals, map_case, cbar_label, scan):
    ...
    if last_sc is not None:
        ...
        cbar_i.set_label("Normalised Intensity [-]", fontsize=11)
...
def plot3_loss_vs_delta(all_data, scan_vals, map_case, cbar_label, scan):
    ...
    ax.set_xlabel(r"$\delta\ [10^{-3}]$", fontsize=12)
    ax.set_ylabel(r"$|dI/d\delta|$", fontsize=12)
    ax.set_title(r"Loss rate vs $\delta$ — " + map_case, fontsize=12)
...
def plot4_tunes_vs_delta(all_data, scan_vals, map_case, cbar_label, scan):
    ...
    ax_qx.set_ylabel(r"$Q_x$", fontsize=12)
    ...
    ax_qy.set_ylabel(r"$Q_y$", fontsize=12)
    ax_qy.set_xlabel(r"$\delta\ [10^{-3}]$", fontsize=12)
